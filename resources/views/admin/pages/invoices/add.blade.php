<div class="col-md-12">
    ...
    <div class="portlet light bordered">
        ...
        <div class="portlet-body ">
            <form class="invoicefrm" method="POST" action="{{ route('admin.invoices.store') }}"
                ...
                <table class="table table-striped table-bordered table-hover table-user" id="table_DT">
                    ...
                    @endphp
                    <div class="form-group col-md-4 mb-5{{ $errors->has('invoice_no') ? ' has-error' : '' }}">
                        ...
                        <div class="input-icon">
                            ...
                            <input type="text" class="form-control" name="invoice_no" id="invoice_no"
                                   placeholder="Enter Invoice No" maxlength="80" required value="DD/{{$last_id}}"
                                   readonly>
                    ...
                    <div class="form-group col-md-4 mb-5 {{ $errors->has('invoice_date') ? ' has-error' : '' }}">
                        ...
                        <div class="input-icon">
                            ...
                            <input type="date" class="form-control" name="invoice_date" id="invoice_date"
                                   placeholder="Enter Invoice Date" maxlength="80" value="{{ old('invoice_date') }}">
                    ...
                    <div class="form-group col-md-4">
                        <label for="cash_debit" class=" control-label">{!! $mend_sign !!}Cash/Debit</label>

                        <select class="form-control cash_debit " name="cash_debit">
                    ...
                    @if(!empty($orders))
                        ...
                        @foreach($orders as $order)
                            ...
                            <tr>
                                <th><input type="checkbox" class="order_class order_{{$order->id}}"
                                           data-order_id="{{$order->id}}" id="order_{{$order->id}}">
                                    @if(empty($order->franchise_id))
                                        <input type="hidden" value="{{$order->user_id}}" name="user_id"/>
                                    @else
                                        <input type="hidden" value="{{$order->franchise_id}}" name="franchise_id"/>
                                ...
                                @if(!empty($order->franchises_id))
                                    <input type="hidden" value="{{$order->franchises_id}}" name="franchise_id"/>
                                @endif                                <th colspan="4">Order No :{{$order->order_no}}</th>

                                <th colspan="3">{{$order->type}} <input type="hidden" name="type"
                                                                        value="{{$order->type}}"/></th>
                                <th colspan="2">Total Amount :{{$order->total_amount}} <input type="hidden"
                                                                                              name="total_amount[{{$order->id}}]"
                            ...
                            @if($order->type=="Normal")
                                ...
                                @foreach($items as $item)
                                    <tr>
                                        <td><input type="checkbox" class="item_{{$order->id}}" id="item_{{$item->id}}"
                                                   name="item_id[{{$order->id}}][]" value="{{$item->id}}">
                                        </td>
                                        <td>{{$item->id}}</td>
                            ...
                            @else
                                ...
                                @if(!empty($customitem))
                                    <tr>
                                        <td><input type="checkbox" class="item_{{$order->id}}"
                                                   id="item_{{$customitem->id}}" name="item_id[{{$order->id}}][]"
                                                   value="{{$customitem->id}}">
                                        <td>{{$customitem->id}}</td>
                                        ...
                                        @endphp
                                        <td>@if($discount) {{$discount}} @else <input type="text"
                                                                                      name="discount[{{$customitem->id}}]"
                                                                                      class="form-control discount"> @endif
                                        </td>
                                        <td>18</td>
                                        <td>9</td>
                                        <td>9</td>
